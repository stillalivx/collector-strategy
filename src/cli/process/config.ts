...
async function config() {
  ...
  if (!Object.keys(configModel).includes(regexMatch[1])) {
    throw new InterfaceError(
      `No existe "${regexMatch[1]}" en la configuración`,
    );
  }

  if (!Object.keys(configModel[regexMatch[1]]).includes(regexMatch[2])) {
    throw new InterfaceError(
      `No existe "${regexMatch[2]}" en la configuración de "${regexMatch[1]}"`,
    );
  }
  ...
  if (regexMatch[3]) {
    if (
      !Object.keys(configModel[regexMatch[1]][regexMatch[2]]).includes(
        regexMatch[3],
      )
    ) {
      throw new InterfaceError(
        `No existe "${regexMatch[3]}" en la configuración de "${
          regexMatch[2]
        }"`,
      );
    }
